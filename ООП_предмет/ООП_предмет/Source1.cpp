...
public:
    ...
    virtual bool IsUserInChat(const string& username) const {
        return find_if(participants.cbegin(), participants.cend(),
            [username](User* user) { return user->getNicName() == username; }) != participants.cend();
    }
